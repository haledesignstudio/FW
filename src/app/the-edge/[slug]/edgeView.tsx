...
interface EdgeScenarioDesktopViewProps {
	data: EdgeScenario;
	slug: string;
}
...
export default function EdgeScenarioView({ data, slug }: EdgeScenarioDesktopViewProps) {
	...
	const carouselItems = (data.articleContents || []).map((item, idx) => ({
		id: String(idx),
				...
				<>
					...
					<main className="p-[2vh] bg-[#F9F7F2]">
						...
						<div className="grid grid-cols-4 gap-y-[1.5vh] gap-y-[4vh] auto-rows-[minmax(0,auto)]">
							...
							{/* Back to top button */}
							<div className="col-span-2 col-start-3 flex justify-end items-center cursor-pointer row-span-1" onClick={handleBackToTop}>
								<FadeInOnVisible>
